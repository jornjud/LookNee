// script.js (Theme: Minimalist, Features: Phase 1)
...
let debtorsCache = []; // Master list of all debtors for the current user
let paymentListener = null;
let currentDebtorIdForPayment = null;
let currentStatusFilter = 'all'; // NEW: For status filtering
...
function showMessage(message, type = 'success') {
    ...
    messageDiv.className = `p-4 mb-3 text-sm text-white rounded-lg shadow-xl flex items-center gap-3 ${color}`;
...
// Function to get Firestore collection path
function getDebtorCollectionPath() {
    // Note: In a real-world app with stricter rules, you would get this from a secure source.
    // For this example, we use a constant App ID, but the user ID makes it user-specific.
    const appId = 'looknee-revamp';
    if (!userId) {
...
// Set persistence to 'local' to remember the user
setPersistence(auth, browserLocalPersistence)
    ...
    .catch((error) => {
        ...
        initializeAuthListener(); // Still try to initialize
    });
...
function initializeAuthListener() {
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            debtorsCache = [];
            renderDebtors(); // Clear table
            updateSmarterDashboard(); // Clear dashboard
        }
    });
}
...
// Map Firebase auth error codes to Thai messages
function mapAuthError(code) {
    const errors = {
        ...
        'auth/weak-password': 'รหัสผ่านสั้นเกินไป (ขั้นต่ำ 6 ตัวอักษร)',
    };
    return errors[code] || 'เกิดข้อผิดพลาดที่ไม่รู้จัก';
}
...
function closeDebtorModalAction() {
    debtorModal.classList.remove('active');
}

// Handle Debtor Form Submission (Add/Edit)
debtorForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!userId) {
        showMessage('เซสชั่นหมดอายุ กรุณาเข้าสู่ระบบใหม่', 'error');
        return;
    }
    ...
    const debtorData = {
        ...
        status: debtorForm.dataset.mode === 'add' ? 'ยังไม่จ่าย' : debtorForm.debtorStatus.value, // Status on form is for edit only
        notes: debtorForm.debtorNotes.value.trim(),
        lastUpdated: serverTimestamp(),
    };

    if (!debtorData.name || isNaN(debtorData.amount) || !debtorData.dateBorrowed || !debtorData.dateDue) {
        showMessage('กรุณากรอกข้อมูลที่จำเป็น (*)', 'error');
        return;
    }
    ...
    try {
        if (debtorForm.dataset.mode === 'add') {
            debtorData.totalPaid = 0; // Initialize payment for new debtors
            ...
            await updateDoc(doc(db, debtorCollectionPath, debtorId), debtorData);
            showMessage('แก้ไขข้อมูลลูกหนี้สำเร็จ!', 'success');
        }
        closeDebtorModalAction();
    } catch (error) {
        console.error("Error saving debtor:", error);
        showMessage(`บันทึกข้อมูลล้มเหลว: ${error.message}`, 'error');
    }
});
...
function loadDebtors() {
    ...
    debtorsListener = onSnapshot(q, (querySnapshot) => {
        ...
        // Initial render uses current filter and search states
        ...
        if (debtorTableBody) debtorTableBody.innerHTML = '<tr><td colspan="6" class="text-center py-10 text-red-500">เกิดข้อผิดพลาดในการโหลดข้อมูล</td></tr>';
    });
}
...
function renderDebtors(searchTerm = '', statusFilter = 'all') {
    ...
    if (displayDebtors.length === 0) {
        debtorTableBody.innerHTML = `<tr><td colspan="6" class="text-center py-10 text-gray-500">ไม่พบข้อมูลลูกหนี้</td></tr>`;
        return;
    }
    ...
    displayDebtors.forEach(debtor => {
        ...
        row.innerHTML = `
            <td class="td-cell">
                ...
                <div class="text-xs text-gray-500">${debtor.contact || 'ไม่มีข้อมูลติดต่อ'}</div>
            </td>
            <td class="td-cell text-right">
                <div class="font-semibold text-gray-800">${(principalAmount).toLocaleString('th-TH', { minimumFractionDigits: 2 })} ${debtor.currency || 'THB'}</div>
            </td>
...
function getStatusTagInfo(status) {
    const map = {
        'จ่ายแล้ว': { text: 'จ่ายแล้ว', cssClass: 'status-green' },
        'จ่ายบางส่วน': { text: 'จ่ายบางส่วน', cssClass: 'status-blue' },
        'ยังไม่จ่าย': { text: 'ยังไม่จ่าย', cssClass: 'status-yellow' },
        'เลยกำหนด': { text: 'เลยกำหนด', cssClass: 'status-red' },
        'หนี้สูญ': { text: 'หนี้สูญ', cssClass: 'status-purple' }
    };
    return map[status] || { text: 'ไม่ระบุ', cssClass: 'status-gray' };
}
...
async function handleDeleteDebtor(e) {
    ...
    const confirmed = await showCustomConfirm('ต้องการลบลูกหนี้รายนี้และประวัติทั้งหมด?');
    if (confirmed) {
        try {
            ...
            showMessage(`ลบข้อมูลไม่สำเร็จ: ${err.message}`, 'error');
        }
    }
}
...
searchInput.addEventListener('input', (e) => {
    renderDebtors(e.target.value, currentStatusFilter);
});
...
function updateSmarterDashboard() {
    ...
    debtorsCache.forEach(d => {
        ...
        if (status !== 'จ่ายแล้ว' && status !== 'หนี้สูญ') {
        ...
        if (dueDate) {
             if (d.dateDue === todayISO && status !== 'จ่ายแล้ว' && status !== 'หนี้สูญ') {
                dueTodayCount++;
            ...
            if (dueDate > today && dueDate <= sevenDaysFromNow && status !== 'จ่ายแล้ว' && status !== 'หนี้สูญ') {
                dueSoonCount++;
    ...
    totalDebtAmountStat.textContent = formatCurrency(totalOutstanding);
    overdueStat.textContent = `${overdueCount} คน`;
    overdueAmountStat.textContent = formatCurrency(overdueAmount);
    dueTodayStat.textContent = `${dueTodayCount} คน`;
    dueTodayAmountStat.textContent = formatCurrency(dueTodayAmount);
    dueSoonStat.textContent = `${dueSoonCount} คน`;
    dueSoonAmountStat.textContent = formatCurrency(dueSoonAmount);
}


// --- Payment History Modal Logic ---
// ... (The payment history logic from your previous script is largely compatible)
// ... I'll include it here for completeness with minor adjustments.
...
async function openPaymentHistoryModal(debtorId, debtorName, principalAmount) {
    ...
    paymentListener = onSnapshot(query(collection(db, paymentsPath)), async (snapshot) => {
                ...
                paymentsHtml += `
                    <li class="p-3 bg-white rounded-lg shadow-sm border border-gray-200">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="font-bold text-lg text-green-600">${amount.toLocaleString('th-TH')} บาท</p>
                                <p class="text-xs text-gray-500">จ่ายเมื่อ: ${pDate}</p>
                                ${p.paymentNote ? `<p class="text-xs text-gray-600 mt-1 pl-2 border-l-2 border-gray-200">"${p.paymentNote}"</p>` : ''}
                            </div>
                            <button class="btn-icon text-red-400 hover:text-red-600 delete-payment" data-payment-id="${p.id}" title="ลบรายการนี้"><i class="fas fa-times-circle"></i></button>
                        </div>
                    </li>`;
            });
            paymentsHtml += '</ul>';
        ...
        paymentsHtml += `
            <div class="mt-4 pt-4 border-t border-gray-200 text-right space-y-1 font-semibold">
                <p>ยอดตั้งต้น: <span class="text-gray-800">${currentPrincipalAmountForPaymentModal.toLocaleString('th-TH')} บาท</span></p>
                <p>รวมจ่ายแล้ว: <span class="text-green-600">${totalPaidForThisDebtor.toLocaleString('th-TH')} บาท</span></p>
                <p>ยอดคงเหลือ: <span class="text-xl ${remainingBalance <= 0 ? 'text-green-600' : 'text-red-600'}">${remainingBalance.toLocaleString('th-TH')} บาท</span></p>
            </div>
        `;
...
async function updateDebtorTotalPaid(debtorId, totalPaid) {
    const debtorDocPath = getDebtorCollectionPath();
    if (!debtorDocPath || !debtorId) return;

    try {
        await updateDoc(doc(db, debtorDocPath, debtorId), {
            totalPaid: totalPaid,
            lastUpdated: serverTimestamp()
        });
        console.log(`อัปเดตยอดจ่ายรวมของลูกหนี้ ${debtorId} เป็น ${totalPaid}`);
    } catch (error) {
        console.error(`เกิดข้อผิดพลาดในการอัปเดตยอดจ่ายรวม:`, error);
    }
}
...
async function handleDeletePayment(e) {
    ...
    const confirmed = await showCustomConfirm('ต้องการลบรายการชำระเงินนี้?');
    if (confirmed) {
        ...
        try {
            ...
            showMessage(`ลบรายการชำระเงินไม่สำเร็จ: ${err.message}`, 'error');
        }
    }
}
...
addPaymentForm.addEventListener('submit', async (e) => {
    ...
    if (isNaN(paymentData.amountPaid) || paymentData.amountPaid <= 0 || !paymentData.datePaid) {
        showMessage('กรุณากรอกข้อมูลการชำระให้ถูกต้อง', 'error');
        return;
    }
    const paymentsPath = getPaymentsCollectionPath(debtorId);
    try {
        ...
        showMessage(`เพิ่มรายการชำระเงินไม่สำเร็จ: ${err.message}`, 'error');
    }
});
...
function createCustomConfirmModal() {
    ...
    // Uses the one in index.html now
...
document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM is ready. Minimalist theme activated.");
